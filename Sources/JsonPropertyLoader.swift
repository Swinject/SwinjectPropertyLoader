...
final public class JsonPropertyLoader {
    ...
    private func stringWithoutComments(_ str: String) -> String {
        let pattern = "(([\"'])(?:\\\\\\2|.)*?\\2)|(\\/\\/[^\\n\\r]*(?:[\\n\\r]+|$)|(\\/\\*(?:(?!\\*\\/).|[\\n\\r])*\\*\\/))"
        let expression = try? RegularExpression(pattern: pattern, options: .anchorsMatchLines)
        
        let matches = expression!.matches(in: str, options: RegularExpression.MatchingOptions(rawValue: 0),
...
extension JsonPropertyLoader: PropertyLoaderType {
    public func load() throws -> [String : AnyObject] {
        ...
        let data = jsonWithoutComments.data(using: .utf8)
